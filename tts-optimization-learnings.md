...
| API mode | Sequential `/runsync` calls |
| Stability | Stable, no timeout issues |

## 6. Parallel TTS Implementation (Success)

### Implementation
- **Pattern:** Sliding window with asyncio + aiohttp
- **Concurrency:** 3 workers (configurable)
- **Retry logic:** 3 attempts per chunk, abort on persistent failure
- **Ordering:** Index-based sorting before concatenation
...
- **Max Workers:** 3-10 (match `max_concurrent` in code)
- **Scaling Strategy:** Request Count (value: 1)
- **Active Workers:** 0 (accept cold starts) or 1-2 (eliminate cold starts, ~$0.50/hr)

### Files Modified
- `parallel_tts.py` - New async orchestrator module
- `tts_generator.py` - Integration, replaced sequential loop
- `requirements.txt` - Added aiohttp, nest_asyncio

### Branch
- `feat/parallel-tts` - https://github.com/MariusWilsch/chatterbox-runpod/commit/e2e9bc5
